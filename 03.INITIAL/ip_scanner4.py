...
def _mdns_direct_query(mdns_map, timeout=2.0):
    """
    mDNS 멀티캐스트(224.0.0.251:5353)로 직접 쿼리.
    DNS 캐시가 비어있을 때 네트워크에서 직접 .local 장치를 탐색.
    """
    import struct, re
...
def build_mdns_map():
    """
    3단계로 .local 이름 -> IPv4 매핑을 구축:
      1) Windows DNS 캐시에서 IPv4 직접 캐시된 항목 수집
      2) IPv6로만 캐시된 이름은 Resolve-DnsName -Type A 로 IPv4 강제 조회
      3) 캐시가 아예 없으면 mDNS UDP 멀티캐스트로 직접 탐색
    반환: { "192.168.0.17": "rp4-nwkim.local", ... }
    """
    mdns_map = {}
    ipv4_entries = {}   # { "rp4-nwkim.local": "192.168.0.17" }
    ipv6_names   = []   # IPv6 로만 캐시된 .local 이름

    if os.name != "nt":
        # Linux/macOS: avahi-browse 로 수집
        try:
            out = subprocess.check_output(
                ["avahi-browse", "-art", "--no-db-lookup"],
    ...
    try:
        ...
        out = subprocess.check_output(
            ["powershell", "-NoProfile", "-Command", ps_v4],
    ...
    # ── Windows Step 2: IPv6만 캐시된 항목 → IPv4 강제 조회 ─
    try:
        ps_v6 = (
            "Get-DnsClientCache | "
            "Where-Object { $_.Entry -like '*.local' } | "
            "Select-Object -ExpandProperty Entry -Unique"
        )
        out = subprocess.check_output(
            ["powershell", "-NoProfile", "-Command", ps_v6],
            stderr=subprocess.DEVNULL, timeout=5
        ).decode(errors="ignore")
        for line in out.strip().split("\n"):
            name = line.strip()
    ...
    for name in ipv6_names:
        try:
            ...
            out = subprocess.check_output(
                ["powershell", "-NoProfile", "-Command", ps_resolve],
                stderr=subprocess.DEVNULL, timeout=4
    ...
    # ── Windows Step 3: 캐시 완전 미스 → UDP 멀티캐스트 직접 탐색 ─
    if not mdns_map:
        _mdns_direct_query(mdns_map)
