...
* 1 ~ 3단계: "누구인지" 확인하고 "쓸 권한"을 획득하는 준비 과정 (이전에 얘기한 §2~3)
* 4단계 (dry_run): 아무 물리 동작 없이 상태만 읽어서 "이 계획이 실행 가능한가"를 미리 검증
* 5~7단계 (run): 실제 명령 전송 → UUID로 추적 → 진행상황 스트림 → 결과 회수
* 8단계: 잠금 해제, 다음 작업은 다시 3번부터 반복
