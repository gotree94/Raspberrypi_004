...
부팅이 완료되면 Python 프로그램(`/home/gotree94/test.py`)을 자동으로 실행하는 4가지 방법을 정리합니다.
...
[Service]
ExecStart=/usr/bin/python3 /home/gotree94/test.py
WorkingDirectory=/home/gotree94
...
```cron
@reboot sleep 10 && /usr/bin/python3 /home/gotree94/test.py >> /home/gotree94/test.log 2>&1
```

> **`sleep 10`** : 부팅 직후 네트워크, GPIO 등 주변 장치가 준비될 때까지 10초 대기  
> **`>> /home/gotree94/test.log 2>&1`** : 표준출력 및 에러를 로그 파일에 저장

### 2-3. 로그 확인

```bash
cat /home/gotree94/test.log
tail -f /home/gotree94/test.log
```
...
```bash
ps aux | grep test.py
journalctl -u rc-local -n 20 --no-pager
```
...
Type=Application
Name=test
Exec=/usr/bin/python3 /home/gotree94/test.py
...
- 사용자: gotree94
- Python 실행 파일: `/home/gotree94/test.py`
