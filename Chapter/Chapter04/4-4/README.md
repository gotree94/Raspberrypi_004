...
* 파란색 영역만 분리한 뒤, 그중에서 가장 큰 파란색 삼각형을 찾아 외곽선과 중심점을 표시하는 프로그램을 작성합니다.
...
* 라즈베리파이 카메라로 영상을 받아 파란색 삼각형을 검출하고, 그 위치(cx)와 크기(area)를 이용해 자동차의 진행 반향(좌,우,직즌,정지)을 판단하는 프로그램을 작성합니다.
...
* 파란색 삼각형을 검출하여 중심 오차를 기바으로 서보 조향각을 제어하고, 목표와의 거리(면적)에 따라 전진 속도를 조절하거나 정지하는 코드를 작성합니다.
