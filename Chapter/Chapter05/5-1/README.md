...
3. 동적 계산 그래프 (Dynamic Graph): requires_grad=True로 설정된 텐서들이 연산을 거치며 어떻게 실시간으로 노드(Tensor)와 엣지(연산, grad_fn)로 이루어진 계산 이력(History)을 <br>
...
1. 자동미분 엔진:
   ...
   * 다변수 그래디언트: 여러 변수($x$, $z$)에 대한 손실 함수(Loss function)의 지형도(3D 맵)를 보여주고, 각 변수에 대한 편미분값들이 어떻게 그래디언트 벡터($\nabla y$)로 합쳐져 최적의 방향을 제시하는지 표현합니다.
3. 최적화 루프 및 학습:
