#!/usr/bin/env python3
"""
Raspberry Pi 4 Controller — STM32F103 Simulator Verification Tool (GUI, ASCII only)
Based on SIMULATOR_DEV_GUIDE.md Stage 12~14 protocol

Usage:
    python rpi_controller_v1.py                           # GUI mode (select port)
    python rpi_controller_v1.py --serial-port COM3        # Direct COM3 connection
    python rpi_controller_v1.py --serial-port COM3 --auto-test  # CLI auto test
"""
...
# ============================================================
# RPi Controller — Backend (serial comm + data processing, ASCII only)
# ============================================================
class RPiController:
    ...
    def connect(self, port=None):
        ...
        if not self.serial_port:
            self._emit_log("[ERROR] No port specified")
            return False
        try:
            import serial
            ...
            self._emit_log(f"{self.serial_port} @ {self.baud}bps connected")
            return True
        except ImportError:
            self._emit_log("[ERROR] pyserial required: pip install pyserial")
            return False
        except Exception as e:
            self._emit_log(f"[ERROR] Connection failed: {e}")
            return False
    ...
    def send_cmd(self, cmd_str):
        if not self.ser or not self.ser.is_open:
            self._emit_log("[ERROR] Not connected")
            return None
        ...
        try:
            ...
            self._emit_log(f"[ERROR] Send failed: {e}")
            return None

    def auto_test(self, progress_cb=None, done_cb=None):
        tests = [
            ("WHEEL1=500", "Wheel1 speed set"),
            ("WHEEL2=300", "Wheel2 speed set"),
            ("WHEEL3=700", "Wheel3 speed set"),
            ("WHEEL4=999", "Wheel4 max speed"),
            ("WHEEL1=0",   "Wheel1 stop"),
            ("SERVO1=0",   "Servo1 0deg"),
            ("SERVO1=500", "Servo1 90deg"),
            ("SERVO1=999", "Servo1 180deg"),
            ("SERVO2=250", "Servo2 45deg"),
            ("CLCD=Hello World!", "CLCD line1 output"),
            ("CLCD=Line1\nLine2", "CLCD line2 output"),
            ("LED_G=500",  "2-color LED G=500"),
            ("LED_B=800",  "2-color LED B=800"),
            ("LED_RGB_R=999", "3-color LED R=999"),
            ("LED_RGB_G=500", "3-color LED G=500"),
            ("LED_RGB_B=200", "3-color LED B=200"),
            ("BUZZER=1",   "Buzzer ON"),
            ("BUZZER=0",   "Buzzer OFF"),
            ("LASER=1",    "Laser ON"),
            ("LASER=0",    "Laser OFF"),
        ...
        error_tests = [
            ("WHEEL1=1000", "Range over"),
            ("SERVO3=500",  "Unknown key"),
            ("BUZZER=2",    "Invalid value"),
            ("INVALID_KEY=1", "Unknown key"),
        ]
        passed = 0
        failed = 0
        results = []
        ...
        for cmd, desc in tests:
            ...
            if found:
                ...
                results.append((desc, cmd, False, "No R,OK"))
                failed += 1
                if progress_cb:
                    progress_cb("", False)
        ...
        for cmd, desc in error_tests:
            ...
            if found:
                ...
                results.append((desc, cmd, False, "No R,ERR"))
                failed += 1
                if progress_cb:
                    progress_cb("", False)
...
class ControllerGUI:
    ...
    def __init__(self, ctrl):
        ...
        self.root = tk.Tk()
        self.root.title("RPi Controller — STM32F103 Simulator Verifier (ASCII)")
    ...
    def _build_ui(self):
        ...
        tk.Label(conn_frame, text="COM port:", bg=self.COLORS['card'],
        ...
        self.conn_status = tk.Label(conn_frame, text="\u26d4 Disconnected", bg=self.COLORS['card'],
        ...
        sensor_frame = tk.LabelFrame(left_frame, text="Sensor Data (Real-time)",
        ...
        self.sensor_labels = {}
        sensors = [
            ('CDS', 'CDS Illuminance'), ('HALL', 'HALL Hall'),
            ('TEMP', 'TEMP Temp'), ('HUMI_T', 'HUMI_T Humi Temp'),
            ('HUMI_H', 'HUMI_H Humidity'), ('ROT', 'ROT Rotary'),
            ('US_DIST', 'US_DIST Ultrasonic'), ('JOY_X', 'JOY_X Joystick X'),
            ('JOY_Y', 'JOY_Y Joystick Y'), ('IR_RX', 'IR_RX Receive'),
            ('GPS_LAT', 'GPS_LAT Latitude'), ('GPS_LNG', 'GPS_LNG Longitude'),
            ('FUEL', 'FUEL Fuel'), ('ACC_X', 'ACC_X Accel X'),
            ('ACC_Y', 'ACC_Y Accel Y'), ('ACC_Z', 'ACC_Z Accel Z'),
            ('RPM', 'RPM Engine'),
        ]
        ...
        cmd_label = tk.Label(ctrl_frame, text="Command Send (ASCII)", fg=self.COLORS['accent'],
        ...
        tk.Button(entry_frame, text="Send", command=self._send_manual,
                  bg=self.COLORS['card'], fg=self.COLORS['accent'],
                  font=('Consolas', 8, 'bold'), width=6).pack(side=tk.RIGHT)

        self.test_btn = tk.Button(ctrl_frame, text="\u25b6 Auto Test (26 items)",
        ...
        tk.Button(clcd_frame, text="Send", command=self._send_clcd,
                  bg=self.COLORS['card'], fg=self.COLORS['accent'],
        ...
        log_frame = tk.LabelFrame(self.root, text="Serial Log (ASCII)",
                                  fg=self.COLORS['accent'], bg=self.COLORS['bg'],
                                  font=('Consolas', 10, 'bold'))
        ...
        self.status_label = tk.Label(
            status_frame, text="Select port and connect | 10Hz | RX: 0 packets | Err: 0",
    ...
    def _toggle_connection(self):
        if self._connected:
            ...
            self._connected = False
            self.conn_btn.config(text="Connect", bg=self.COLORS['green'])
            self.conn_status.config(text="\u26d4 Disconnected", fg=self.COLORS['red'])
            return
        ...
        if self.ctrl.connect(dev):
            ...
            self.conn_btn.config(text="Disconnect", bg=self.COLORS['red'])
    ...
    def _start_sensor_update(self):
        def loop():
            ...
            port_display = self.ctrl.serial_port or "Not connected"
            self.status_label.config(
                text=f"{port_display} | {self.ctrl.baud}bps | RX: {pkt} packets")
            self.root.after(100, loop)
        self.root.after(100, loop)

    def _start_auto_test(self):
        if not self._connected:
            messagebox.showwarning("Notice", "Connect first")
            return
    ...
    def _test_done(self, passed, failed, results):
        ...
        if failed == 0:
            self.test_log.insert(tk.END, f"\u2705 All passed: {passed}/{passed + failed}\n", 'pass')
        else:
            self.test_log.insert(tk.END, f"\u26a0\ufe0f  {passed} passed, {failed} failed\n", 'fail')
        ...
        self.test_log.see(tk.END)
        self.test_log.config(state=tk.DISABLED)
        self.test_btn.config(text="\u25b6 Auto Test (26 items)", state=tk.NORMAL)
        messagebox.showinfo("Test Complete",
                            f"{'All passed' if failed == 0 else 'Some failed'}\n"
...
def main():
    ...
    parser.add_argument('--serial-port', default=None, help='Serial port')
    parser.add_argument('--baud', type=int, default=115200, help='Baud rate')
    parser.add_argument('--auto-test', action='store_true', help='CLI auto test')
    ...
    if args.auto_test:
        ...
        if not port:
            ports = scan_serial_ports()
            if not ports:
                print("[ERROR] No available COM ports")
                return
