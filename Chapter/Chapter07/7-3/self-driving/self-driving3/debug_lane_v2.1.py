...
def draw_dashed_line(img, pt1, pt2, color, thickness=2, dash_len=10, gap_len=6):
    ...
    while d < length:
        s = d
        e = min(d + dash_len, length)
        p1 = (int(pt1[0] + ux * s), int(pt1[1] + uy * s))
        p2 = (int(pt1[0] + ux * e), int(pt1[1] + uy * e))
        cv2.line(img, p1, p2, color, thickness)
        d += dash_len + gap_len


def draw_dashed_polygon(img, pts, color, thickness=2, dash_len=10, gap_len=6):
    for i in range(len(pts)):
        draw_dashed_line(img, pts[i], pts[(i + 1) % len(pts)], color, thickness, dash_len, gap_len)
...
def detect_and_draw(frame):
    ...
    def filter_outliers(points, max_dev=15):
        if len(points) < 4:
            return points, []
        pts = np.array(points)
        ...
        deviations = np.abs(xs - predicted)
        median_dev = np.median(deviations)
        if median_dev < 1:
            median_dev = 1
        mask = deviations < max(max_dev, median_dev * 3)
        filtered = [points[i] for i in range(len(points)) if mask[i]]
        outliers = [points[i] for i in range(len(points)) if not mask[i]]
        return filtered, outliers

    left_inner, left_outliers = filter_outliers(left_inner)
    right_inner, right_outliers = filter_outliers(right_inner)

    for px, py in left_outliers + right_outliers:
        cv2.drawMarker(debug_img, (int(px), int(py)), (0, 0, 255),
                       cv2.MARKER_TILTED_CROSS, 12, 2)
    for lx, ly in left_inner:
        cv2.circle(debug_img, (int(lx), int(ly)), 6, (0, 255, 255), -1)
    for rx, ry in right_inner:
        cv2.circle(debug_img, (int(rx), int(ry)), 6, (255, 0, 255), -1)

    if left_outliers or right_outliers:
        print(f"    Outliers removed: L={len(left_outliers)} R={len(right_outliers)}")
    print(f"  After filter: L={len(left_inner)} R={len(right_inner)}")
    ...
    if len(left_inner) >= 3 and len(right_inner) >= 3:
        ...
        trap_pts = [
            (int(lx_arr[0]), int(ys_arr[0])),
            (int(rx_arr[0]), int(ys_arr[0])),
            (int(rx_arr[-1]), int(ys_arr[-1])),
            (int(lx_arr[-1]), int(ys_arr[-1])),
        ]
        draw_dashed_polygon(debug_img, trap_pts, (0, 255, 255), thickness=2, dash_len=12, gap_len=8)
