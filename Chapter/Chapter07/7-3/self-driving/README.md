...
> **그림 설명**: Ground Truth 대비 예측 오차(예측값 - 실제값) 분포. 빨간 점선은 평균 오차, 검은 점선은 이상적인 0위치. 분포가 0에 가까울수록 정확도가 높음. 모든 모델에서 약간의 양의 편향(+15°~16°)이 존재하며, 이는 Ground Truth 자체가 이전 모델 앙상블에 의해 생성된 값이기 때문.
